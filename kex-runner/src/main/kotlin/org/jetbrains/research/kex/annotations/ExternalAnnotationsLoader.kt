...
class ExternalAnnotationsLoader : AnnotationsLoader {
    ...
    fun parseAnnotations(param: Element): List<AnnotationInfo> {
        val result = mutableListOf<AnnotationInfo>()
        val nodes = param.getElementsByTagName("annotation")
        var i = 0
    ...
    private fun loadAnnotations(stream: InputStream, file: String) = try {
        ...
        for (node in generateSequence { if (i == nodes.length) null else nodes.item(i++) }) {
            ...
            val paramsAnnotations = mutableMapOf<String, List<AnnotationInfo>>()
            val parameteres = node.getElementsByTagName("parameter")
            var i = 0
            try {
                for (param in generateSequence { if (i == parameteres.length) null else parameteres.item(i++) as Element })
                    paramsAnnotations[param.attributes.getNamedItem("index").nodeValue] =
                            parseAnnotations(param)
            } catch(thr: Throwable) {
